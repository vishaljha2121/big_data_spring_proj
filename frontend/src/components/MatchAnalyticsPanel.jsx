...
import { compactId, fixed, pct } from "../utils/formatting.js";
    ...
    <GlassCard className="match-analytics">
      <div className="panel-heading">
        <div>
          ...
          <h2>{summary.player_a || "Player A"} <span>vs</span> {summary.player_b || "Player B"}</h2>
          <p>Online scoring replay for the selected synthetic match.</p>
        </div>
        <code className="id-pill" title={summary.synthetic_match_id}>{compactId(summary.synthetic_match_id)}</code>
      </div>
        ...
        {recent.map((event) => (
          <div className="recent-event-row" key={event.event_id}>
            ...
            <div>
              ...
              <small>{event.primary_risk_signal || "No primary signal"}</small>
            </div>
